...
class DepthwiseSeparableConv(nn.Module):
    def __init__(self, input_channel, output_channel, kernel_size, stride=1):
        ...
        # Depthwise Convolution: Apply a separate filter to each input channel
        self.depthwise = nn.Conv2d(
            in_channels=input_channel,
            out_channels=input_channel,  # Same number of output channels as input channels
            kernel_size=kernel_size,
            stride=stride,
            padding=kernel_size // 2,  # Padding to preserve the spatial size
            groups=input_channel,  # Separate filter for each input channel
            bias=False  # Typically, no bias is used in depthwise convolutions
        )

        # Batch Normalization for depthwise convolution
        self.depthwise_bn = nn.BatchNorm2d(input_channel)

        # Pointwise Convolution: 1x1 convolution to combine the outputs of depthwise convolution
        self.pointwise = nn.Conv2d(
            ...
            stride=1,
            padding=0,
            bias=False  # No bias in pointwise convolution
        )

        # Batch Normalization for pointwise convolution
        self.pointwise_bn = nn.BatchNorm2d(output_channel)

        # ReLU activation after the pointwise convolution
        self.relu = nn.ReLU(inplace=True)

    def forward(self, x):
        # Apply depthwise convolution followed by batch normalization and ReLU
        x = self.depthwise(x)
        x = self.depthwise_bn(x)
        x = self.relu(x)

        # Apply pointwise convolution followed by batch normalization and ReLU
        x = self.pointwise(x)
...
class ResBlock_Norml(nn.Module):
    def __init__(self,input_channel,output_channel,stride=1):#输入输出以及stride采样间隔
        ...
        self.conv_1=DepthwiseSeparableConv(input_channel,output_channel,kernel_size=1,stride=1)
...
class ResBlock(nn.Module):
    def __init__(self,input_channel,output_channel,stride=1):#输入输出以及stride采样间隔
        ...
        # 残差块的第一个卷积
        # 通道数变换in->out，每一层（除第一层外）的第一个block
        self.conv_1 = DepthwiseSeparableConv(input_channel,output_channel,kernel_size=3,stride=1)#
...
class Attention_block(nn.Module):
    def __init__(self,F_g,F_l,F_int):
        ...
        self.W_g = nn.Sequential(
            DepthwiseSeparableConv(F_g,F_int,kernel_size=1,stride=1),
...
class InvertedResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, expansion_factor=6):
        ...
        # Depthwise Convolution (3x3 Conv)
        self.depthwise = nn.Conv2d(in_channels * expansion_factor, in_channels * expansion_factor, 
                                    kernel_size=3, stride=stride, padding=1, groups=in_channels * expansion_factor, bias=False)
        self.depthwise_bn = nn.BatchNorm2d(in_channels * expansion_factor)

        # Pointwise Convolution (1x1 Conv)
        self.pointwise = nn.Conv2d(in_channels * expansion_factor, out_channels, kernel_size=1, stride=1, padding=0, bias=False)
...
class DecoderBlock_Conv1(nn.Module):
    def __init__(self, inC, trpC, midC, outC, use_bias=True,
        ...
        # Define upsampling method (either transposed convolution or bilinear upsampling)
        if decoder_type == 'transposed':
            self.up_scale = nn.ConvTranspose2d(in_channels=inC,
                                               ...
                                               bias=use_bias,
                                               dilation=trpD,
                                               padding_mode='zeros')  # Use transposed convolution for upsampling
        elif decoder_type == 'bilinear':
            self.up_scale = nn.Upsample(scale_factor=scale_factor,
                                        mode='bilinear',
                                        align_corners=None)  # Use bilinear upsampling
        else:
            raise NotImplementedError

        # Replacing traditional Conv2d with InvertedResidualBlock
        # InvertedResidualBlock will handle the feature extraction and skip connections
        self.conv_1 = InvertedResidualBlock(in_channels=trpC*2,  # Input channels after concatenation
                                            out_channels=outC,   # Output channels of the block
                                            stride=1,            # Keep the stride as 1
                                            expansion_factor=6)  # Use a higher expansion factor for more feature extraction

        # Attention mechanism can still be applied after the residual block
        self.attention = Augmented_Attention_block(F_g=trpC, F_l=trpC, F_int=trpC)

    def forward(self, input_, skip):  # Using skip connections
        # Perform upsampling (either transposed convolution or bilinear)
        output = self.up_scale(input_)

        # Apply attention to the skip connection based on the upscaled output
        skip = self.attention(output, skip)

        # Ensure that the skip and upscaled output have the same spatial dimensions (if needed)
        if skip.size(2) != output.size(2) or skip.size(3) != output.size(3):
            skip = F.interpolate(skip, size=output.shape[2:], mode='bilinear', align_corners=False)

        # Concatenate the skip connection and the upscaled output
        output = torch.cat([skip, output], dim=1)

        # Pass through the InvertedResidualBlock for feature extraction
...
class FinalConv(nn.Module):
    def __init__(self, inC, outC, use_bias=True,
                 outK=3, outS=1, outP=1, outD=1, outG=1):
...
class submission_20214173(nn.Module):
    def __init__(self, in_channels=1, num_classes=1):
        super(submission_20214173, self).__init__()#继承自父类的属性进行初始化
        ...
        self.init_conv = InitConv(inC=in_channels, midC=init_conv_mid_ch,
        ...
        self.final_conv = FinalConv(inC=decode_4_out_ch, outC=num_classes,
                                    use_bias=True,
                                    outK=3, outS=1, outP=1, outD=1, outG=1)
    ...
    def _same_padding(self,input_):
        ...
        output = F.pad(input=input_, pad=pad_parten,
                               mode='constant', value=0)
        return output
# RHACrackNet(1)(torch.rand((1, 1, 384, 512))).shape
# submission.py (models/seaformer/submission.py)